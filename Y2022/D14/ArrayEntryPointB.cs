...
public class ArrayEntryPointB : IArrayEntryPoint
{
    ...
    private static void PourSand()
    {
        while (true)
        {
            if (IsSpotOccupied(PouringSpot)) return;
    ...
    {
        if (unit.Move(Down).Z == MaxZ) return false;
    ...
    {
        var minX = Math.Min(Rocks.Min(x => x.X), Sand.Min(x => x.X));
        var maxX = Math.Max(Rocks.Max(x => x.X), Sand.Max(x => x.X));
        const int minZ = 0;
        var maxZ = MaxZ;
        Console.Write("xxx  ");
